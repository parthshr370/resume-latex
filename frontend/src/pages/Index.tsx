...
export default function IndexPage() {
    ...
    const [results, setResults] = useState<{ json_str: string, latex_str: string, pdf_b64: string } | null>(null);
                ...
                <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <div className="flex items-center gap-2">
                            <Button variant="outline" onClick={() => handleDownload(results.pdf_b64, 'resume.pdf', 'application/pdf', true)}>
                    ...
                    <TabsContent value="pdf">
                        <Card className="bg-card border-border">
                            <CardContent className="p-2">
                                <object data={`data:application/pdf;base64,${results.pdf_b64}`} type="application/pdf" className="w-full h-[80vh] border rounded-md">
                                    <p>Your browser does not support PDF previews. You can <Button variant="link" onClick={() => handleDownload(results.pdf_b64, 'resume.pdf', 'application/pdf', true)}>download the PDF</Button> instead.</p>
                                </object>
                            </CardContent>
                        </Card>
                    </TabsContent>
                    <TabsContent value="latex">
                        <Card>
                            <CardContent className="p-2">
                                <Textarea value={results.latex_str} rows={30} readOnly className="bg-input border-border font-mono text-xs"/>
                            </CardContent>
                        </Card>
                    </TabsContent>
                    <TabsContent value="json">
                        <Card>
                            <CardContent className="p-2">
                                <Textarea value={JSON.stringify(JSON.parse(results.json_str), null, 2)} rows={30} readOnly className="bg-input border-border font-mono text-xs"/>
                            </CardContent>
                        </Card>
                    </TabsContent>
                </Tabs>
    ...
    if (!showForm) {
        return (
            <BlueprintSection title="Resume Blueprint" description="Follow the steps to build a professional resume.">
                <div>Content for blueprint...</div>
            </BlueprintSection>
        );
    }
            ...
            <div className="container mx-auto p-4 sm:p-6 lg:p-8">
                <header className="flex items-center justify-between mb-8">
                    <div>
                        ...
                        <p className="text-muted-foreground">Fill out the sections to create your professional resume.</p>
                    </div>
                    {!results && (
